...
function ClaimedCondition:tostring()
    local target_names = L(self.claim_ids):map(function(claim_id)
        return windower.ffxi.get_mob_by_id(claim_id)
    end):compact_map():map(function(mob) return mob.name end)
